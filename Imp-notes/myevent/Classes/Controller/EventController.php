...
class EventController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController {
	...
	public function eventAction() {
		...
		if (isset($getData['allevent']) && $getData['allevent'] > 0) {
			$allevent = intval($getData['allevent']);
		}
		$template = $this->eventService->missingConfiguration($this->settings);
		$data = $this->eventRepository->getEventData($this->settings, $detail,$allevent);
		$paginate = 0;
		...
		$this->view->assign('allevent', $allevent);
