...
class ProductController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController {
	...
	public function listAction() {
		...
		$id = $this->settings['categoriesList'];
		$pro_id = $this->settings['categoriesList'];
		if($_GET[tx_product_product][category]){
	...
	public function menuAction() {
	    ...
	    $main_categories = $this->productRepository->findMenucategory();
	               ...
	               foreach($first_menu as $key => $value){
                        $first_menu_submenu = $this->productRepository->findMenusub($value['uid']);
                        if($first_menu_submenu){
                        	foreach($first_menu_submenu as $menu){
                        $first_menu[$key]['sub_menu'][$menu['uid']]['id'] = $menu['uid'];
                      	$first_menu[$key]['sub_menu'][$menu['uid']]['menu'] = $menu['title'];
                      }
                      }
                    }
	               ...
	               foreach($second_menu as $key => $value){
                        $second_menu_submenu = $this->productRepository->findMenusub($value['uid']);
                        if($second_menu_submenu){
                        	foreach($second_menu_submenu as $menu){
                        $second_menu[$key]['sub_menu'][$menu['uid']]['id'] = $menu['uid'];
                      	$second_menu[$key]['sub_menu'][$menu['uid']]['menu'] = $menu['title'];
                      }
                      }
                    }
