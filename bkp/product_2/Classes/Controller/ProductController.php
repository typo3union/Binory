...
class ProductController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController
{
    ...
    public function listAction()
    {
         ...
         $where1 = 'deleted=0 AND hidden=0 AND sys_language_uid =' . $GLOBALS['TSFE']->sys_language_uid;
          $prodt = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('*','tx_product_domain_model_product',$where1);
         foreach ($prodt as $key => $value) {
            $proid['pro'][] = $value['uid'];
         }
          $prod = implode(',', $proid['pro']);

        $where23 = 'uid_local IN(' . $prod . ') ';
        $b1 =  $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('*','tx_product_product_category_mm',$where23);

        foreach ($b1 as $key => $value) {
            $arrr1[$value['uid_foreign']] = $value['uid_foreign'];
        }

        $where22 = ' AND uid in ('.implode(",",$arrr1).' ) ';
           
        $where33 = 'deleted=0 '.$where22.' AND hidden=0 AND sys_language_uid =' . $GLOBALS['TSFE']->sys_language_uid;
        $c1 = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('*','tx_product_domain_model_category',$where33,'','crdate desc');
        $this->view->assign('categories', $c1);
	   ...
	   $featureproduct = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('*','tx_product_domain_model_product',$where);
        ...
        if (isset($id)) {
            ...
            $where4 = 'deleted=0  AND featured_product=0 AND hidden=0 AND sys_language_uid =' . $GLOBALS['TSFE']->sys_language_uid;
            $products = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('*','tx_product_domain_model_product',$where4,'','crdate desc');
